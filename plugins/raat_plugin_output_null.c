...
typedef struct {
    ...
    double                       clock_rate; // NOTE: this is used for testing clock drift, and should not be used in real life
...
static RC__Status output_get_info(void *vself, json_t **out_info) {
    NullOutputPlugin *self = (NullOutputPlugin*)vself;
...
static void update_signal_path(NullOutputPlugin *self) {
...
static RC__Status output_get_supported_formats(void *vself, RC__Allocator *alloc, size_t *out_nformats, RAAT__StreamFormat/*?*/ **out_formats) {
    ...
    json_t *j_supported_formats = json_object_get(self->config, "supported_formats");
    if (j_supported_formats) {
        ...
        if (!json_is_array(j_supported_formats)) {
            ...
            status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
        }

        size_t              nformats = (size_t)json_array_size(j_supported_formats);
        ...
        json_array_foreach(j_supported_formats, index, value) {
            ...
            if (!j_sample_type) {
                RAAT__ERROR("[output/alsa] while parsing supported_formats, missing required field 'sample_type'");
                status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
            }
            if (!j_sample_rate) {
                RAAT__ERROR("[output/alsa] while parsing supported_formats, missing required field 'sample_rate'");
                status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
            }
            if (!j_bits_per_sample) {
                RAAT__ERROR("[output/alsa] while parsing supported_formats, missing required field 'bits_per_sample'");
                status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
            }
            if (!j_channels) {
                RAAT__ERROR("[output/alsa] while parsing supported_formats, missing required field 'channels'");
                status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
            }
            ...
            if (!strcmp(s_sample_type, "pcm")) {
                ...
                RAAT__ERROR("[output/null] while parsing supported_formats, got invalid sample type '%s'", s_sample_type == NULL ? "(null)" : s_sample_type);
                status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
            }
            ...
            if (sample_rate == 0) {
                RAAT__ERROR("[output/null] while parsing supported_formats, got invalid sample_rate");
                status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
            }
            if (bits_per_sample == 0) {
                RAAT__ERROR("[output/null] while parsing supported_formats, got invalid bits_per_sample");
                status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
            }
            if (channels == 0) {
                RAAT__ERROR("[output/null] while parsing supported_formats, got invalid channels");
                status = RAAT__OUTPUT_PLUGIN_STATUS_INVALID_CONFIG; goto fail;
            }

            formats[index].sample_type     = sample_type;
            formats[index].bits_per_sample = bits_per_sample;
            formats[index].sample_rate     = sample_rate;
            formats[index].channels        = channels;

            RAAT__TRACE("[output/null] [hardcoded] supports %s format %d/%d/%d", s_sample_type, sample_rate, bits_per_sample, channels);
...
typedef struct {
    NullOutputPlugin *self;
    int                token;
} NullOutputThreadState;
...
retry:
    ...
    if (self->state != IDLE) {
        RAAT__TRACE("[null] kicking off old output in force teardown");
        needs_join             = true;
        self->state            = IDLE;
    ...
    if (needs_join) {
        RAAT__TRACE("[null] joining thread in force teardown");
    ...
    if (self->current_token != token) {
        RAAT__TRACE("[null] force teardown required a retry");
        goto retry;
    }
...
static void null_output_thread(void *vself) {
    NullOutputThreadState *threadstate = vself;
    NullOutputPlugin      *self        = threadstate->self;
    ...
    while (true) {
        ...
        if (threadstate->token == self->current_token) {
            ...
            if (self->new_stream && self->stream) {
                ...
                if (now + ns_per_buf > start_time) {
                    RAAT__TRACE("[null] starting playback: now (%lldns) + ns_per_buf(%lldns) = %lldns > %lldns streamsample=%lld", now, ns_per_buf, now+ns_per_buf, start_time, self->start_streamsample);
                    ...
                    RAAT__TRACE("[null] waiting for start time...");
                }
            } else {
                stream              = self->stream;
        ...
        if (stream != NULL && self->resync_delay_remaining_ns <= 0) {
            //if (correction_samples != 0) RAAT__TRACE("[null] applying %d sample correction", correction_samples);
        ...
        // for null output, just sleep the time away
        int64_t target_time = now_raw_ns + ns_per_buf;
        int sleep_us = (int)((target_time - now_raw_ns) / 1000);

        // vvv NOTE: this is used for testing clock drift, and should not be used in real life vvv
        if (self->clock_rate != 1.0) {
            sleep_us = sleep_us / self->clock_rate;
        }
        // ^^^ NOTE: this is used for testing clock drift situations, and should not be used in real life ^^^
        ...
        int64_t before_sleep_us = RC__now_us();
        int64_t corrected_sleep_us = sleep_us - sleep_error;
        RC__usleep(corrected_sleep_us);
        int64_t after_sleep_us = RC__now_us();
        int64_t actual_sleep_us = after_sleep_us - before_sleep_us;

        sleep_error = actual_sleep_us - corrected_sleep_us;
    }
...
static void output_setup(void *vself, RAAT__StreamFormat *format, RAAT__OutputSetupCallback cb_setup, void *cb_setup_userdata, RAAT__OutputLostCallback cb_lost, void *cb_lost_userdata) {
    ...
    if (self->state != IDLE) {
        RAAT__TRACE("[null] kicking off old output");
        needs_join             = true;
        self->state            = IDLE;
        old_cb_lost            = self->cb_lost;
        old_cb_lost_userdata   = self->cb_lost_userdata;
    ...
    if (needs_join) {
        RAAT__TRACE("[null] joining thread");
        uv_thread_join(&self->tid);
    }
    ...
    if (self->current_token != token) {
        RAAT__TRACE("null output setup: lost the race");
    ...
    RAAT__TRACE("null output setup: format is %s", formatstr);
    ...
    if (RC__STATUS_IS_SUCCESS(status)) {
        ...
        if (found) {
            RAAT__TRACE("[null] opening %d/%d/%d", pcmformat.sample_rate, pcmformat.bits_per_sample, pcmformat.channels);
            // OK. we are initialized. Now do the non-null stuff
            ...
            NullOutputThreadState *threadstate = RC__new0(RC__ALLOCATOR_DEFAULT, NullOutputThreadState, 1);
            threadstate->self  = self;
            threadstate->token = token;
            uv_thread_create(&self->tid, null_output_thread, threadstate);
...
static int64_t LOCKED_get_local_time(NullOutputPlugin *self) {
...
output_send_message(void *vself, json_t *message) {
    ...
    RAAT__TRACE("[null] GOT MESSAGE %s", s);
    free(s);
...
RAAT__null_output_plugin_new(RC__Allocator *alloc, RAAT__Device *device, json_t *config, RAAT__OutputPlugin **out_output) {
    ...
    NullOutputPlugin *self            = RC__new0(alloc, NullOutputPlugin, 1);
    if (self == NULL) return RC__STATUS_OUT_OF_MEMORY;

    // initialize the state that is associated with NullOutputPlugin 
    self->alloc                        = alloc;
    ...
    RAAT__TRACE("[null] initializing output");
    ...
    RAAT__TRACE("[null] preferred buffer duration=%fs", self->buffer_duration_secs);
    ...
    RAAT__TRACE("[null] resync delay=%fs", self->resync_delay_secs);
    ...
    RAAT__TRACE("[null] max dsd rate=%d", self->max_dsd_rate);

    // vvv NOTE: this is used for testing clock drift, and should not be used in real life vvv
    json_t *clock_rate = json_object_get(config, "clock_rate");
    if (clock_rate) self->clock_rate = json_number_value(clock_rate);
    if (self->clock_rate == 0) self->clock_rate = 1.0;
    RAAT__TRACE("[null] clock rate=%f", self->clock_rate);
    // ^^^ NOTE: this is used for testing clock drift situations, and should not be used in real life ^^^
    ...
    RAAT__TRACE("[output/null] initialized");
    *out_output = &self->plugin;
    return RC__STATUS_SUCCESS;
}

void
RAAT__null_output_plugin_delete(RAAT__OutputPlugin *output) {
    NullOutputPlugin *self = (NullOutputPlugin*)output;
