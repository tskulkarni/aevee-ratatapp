...
static void parse_sockaddr(lua_State* L, struct sockaddr_storage* address, int addrlen) {
  ...
  if (address->ss_family == AF_INET) {
    ...
    lua_Integer cache_key = ((lua_Integer)addrin->sin_addr.s_addr << 16) | (lua_Integer)port;           

    lua_pushlightuserdata(L, &registry_key_ptr);
    ...
    if (lua_isnil(L, -1)) {
      ...
      lua_rawset(L, LUA_REGISTRYINDEX);       // set sockaddr cache table into the registry
    }

    // now, sockaddr cache table is on stack top
    lua_rawgeti(L, -1, cache_key);    // push cached addr table or nil
    if (!lua_isnil(L, -1)) {            // if we got one, we are all set. just return it
        lua_remove(L, -2);              // remove sockaddr cache table
        return;
    } else {
      lua_remove(L, -1);              // remove nil value
      ...
      lua_rawseti(L, -3, cache_key);                    // set it into sacache (popping one of the values)
      lua_remove(L, -2);                                // remove sacache from the stack
    }
