...
#ifdef _WIN32
// luv downcasts from int64 -> int32 a ton. It sucks, but it doesn't cause problems. On windows it makes dozens of warnings, so quiet them.
...
static const luaL_Reg luv_functions[] = {
  ...
  {"listen", luv_listen},
  {"accept", luv_accept},
  {"read_start_noalloc", luv_read_start_noalloc},
  ...
  {"udp_recv_start_noalloc", luv_udp_recv_start_noalloc},
