...
static void luv_udp_recv_cb(uv_udp_t* handle, ssize_t nread, const uv_buf_t* buf, const struct sockaddr* addr, unsigned flags) {
  ...
  if (buf) {
      if (data->mempool) {
          ...
          data->mempool      = buf->base;
          data->mempool_size = buf->len;
      }
  }
  ...
  if (flags & UV_UDP_PARTIAL) {
    static int partial_table_ptr = 0;
    lua_pushlightuserdata(L, &partial_table_ptr);
    lua_rawget(L, LUA_REGISTRYINDEX);
    if (lua_isnil(L, -1)) {
      lua_remove(L, -1);                        // remove nil value
      lua_newtable(L);
      lua_pushboolean(L, 1);
      lua_setfield(L, -2, "partial");
      lua_pushlightuserdata(L, &partial_table_ptr);
      lua_pushvalue(L, -2);                     // make a copy of the table for lua_rawset
      lua_rawset(L, LUA_REGISTRYINDEX);         // set partial flag table into the registry
    }
  } else {
    static int empty_table_ptr   = 0;
    lua_pushlightuserdata(L, &empty_table_ptr);
    lua_rawget(L, LUA_REGISTRYINDEX);
    if (lua_isnil(L, -1)) {
      lua_remove(L, -1);                        // remove nil value
      lua_newtable(L);
...
static void luv_udp_recv_noalloc_cb(uv_udp_t* handle, ssize_t nread, const uv_buf_t* buf, const struct sockaddr* addr, unsigned flags) {
  lua_State* L = luv_state(handle->loop);
  luv_handle_t* data = handle->data;
  ...
  // data
  if (nread > 0) {
    lua_pushlightuserdata(L, buf->base);
  ...
  // free buf _now_ that we don't need it 
  if (buf) {
      if (data->mempool) {
          free(buf->base);
      } else {
...
static void luv_udp_alloc_cb(uv_handle_t* handle, size_t suggested_size, uv_buf_t* buf) {
  luv_handle_t* data = handle->data;
  if (data->mempool && data->mempool_size >= suggested_size) {
      buf->base = data->mempool;
      buf->len  = data->mempool_size;
      data->mempool      = NULL;
      data->mempool_size = 0;
  } else {
...
static int luv_udp_recv_start_noalloc(lua_State* L) {
  uv_udp_t* handle = luv_check_udp(L, 1);
  int ret;
  luv_check_callback(L, handle->data, LUV_RECV, 2);
  ret = uv_udp_recv_start(handle, luv_udp_alloc_cb, luv_udp_recv_noalloc_cb);
  if (ret < 0) return luv_error(L, ret);
  lua_pushinteger(L, ret);
  return 1;
}
